...
class ExchangeTransactionTypes(models.Model):
    ...
    from_account_type_id = fields.Many2one(
        'exchange.config.accounts', string='Account From', required=True)
    to_account_type_id = fields.Many2one(
        'exchange.config.accounts', string='Account To', required=True)
...
#    show_sched_to_dest = fields.Boolean('show_sched_to_dest')
    ...
    is_loan = fields.Boolean('Is a loan transaction', help='Is a loan/grant transaction type')
    """ TODO
...
#  TODO amount_from = fields.Float('Amount from', required=False, default='exchange.transaction.amount_from')
    ...
    account_from = fields.Many2one(
         'Account from', related='transaction_id.account_from_id', readonly=True)
    account_to = fields.Many2one(
         'Account to', related='transaction_id.account_to_id', readonly=True)
