...
class VoteCategory(orm.AbstractModel):
    ...
    """
    Abstract model for exchange_rating category,
    used for specifying different exchange_rating type depending on the object category
    """

    _name = 'exchange_rating.category'
    _description = 'Feedback Category'

    _inherit = 'base.config.inherit.model'
    _base_config_inherit_model = 'exchange_rating.config.line'
    ...
    _columns = {
        'vote_config_ids': fields.one2many(
            'exchange_rating.config.line', 'res_id',
            domain=lambda self: [
    ...
    def _get_external_config(self, cr, uid, record, context=None):
        # Get exchange_rating type from configuration
        res = {}
        vote_config_obj = self.pool.get('exchange_rating.config.line')
...
# )
#
#            for exchange_rating in vote_obj.browse(cr, uid, vote_ids, context=context):
#                vote_average.append(exchange_rating.exchange_rating)
#                res[record.id]['vote_total'] += 1
#                res[record.id]['vote_user_ids'].append(exchange_rating.user_id.id)
...
# sum(vote_average)/len(vote_average)
    ...
    def _get_vote_voters(self, cr, uid, ids, name, args, context=None):
        # Get all partner who exchange_rating on this record
        res = {}
    ...
    def _get_vote_config(self, cr, uid, ids, context=None):
        # Get exchange_rating type from configuration or category
        vote_config_obj = self.pool.get('exchange_rating.config.line')
        res = {}
        for record in self.browse(cr, uid, ids, context=context):
            res[record.id] = []
    ...
    _columns = {
        'vote_average': fields.function(
            _get_vote_stats, type='float', string='Average exchange_rating',
        ...
        'vote_vote_ids': fields.one2many(
            'exchange_rating.exchange_rating', 'res_id',
...
class VoteVote(orm.Model):
    ...
    """
    Object containing the exchange_rating
    """

    _name = 'exchange_rating.exchange_rating'
    _description = 'Feedback'

    def _get_lines(self, cr, uid, ids, name, value, arg, context={}):
        # Compute some field linked the lines of the exchange_rating
        res = {}
        type_obj = self.pool.get('exchange_rating.type')
    ...
    def _get_res_names(self, cr, uid, ids, name, value, arg, context={}):
        ...
        # to give some insight to the viewer of the exchange_rating
        res = {}
    ...
    _columns = {
        ...
        'line_ids': fields.one2many('exchange_rating.exchange_rating.line', 'vote_id', 'Lines'),
        ...
        'evaluated_object_ids': fields.many2many(
            'exchange_rating.evaluated', 'vote_vote_evaluated_rel',
            'vote_id', 'evaluated_id', 'Evaluated'
        )
    }

    def _default_line_ids(self, cr, uid, context={}):
        # Get the exchange_rating line from model config
    ...
    def _update_evaluated(self, cr, uid, ids, context=None):
        # Update the list of evaluated record linked to this exchange_rating
        for vote in self.browse(cr, uid, ids, context=context):
...
class VoteVoteLine(orm.Model):
    ...
    """
    Vote line, containing the numeric exchange_rating for each exchange_rating type
    """

    _name = 'exchange_rating.exchange_rating.line'
    ...
    _columns = {
        ...
        'vote_id': fields.many2one('exchange_rating.exchange_rating', 'Vote', ondelete='cascade'),
        'type_id': fields.many2one('exchange_rating.type', 'Type'),
        'exchange_rating': fields.selection([('-2', '-2'),
                                  ('-1', '-1'),
                                  ('0', '0'),
                                  ('1', '1'),
                                  ('2', '2')], 'Vote')
...
class VoteEvaluated(orm.Model):
    ...
    _name = 'exchange_rating.evaluated'

#    _columns = {
#        'vote_evaluated_ids': fields.many2many(
#            'exchange_rating.exchange_rating', 'vote_vote_evaluated_rel',
...
class ResPartner(orm.Model):
    ...
    _inherits = {'exchange_rating.evaluated': "vote_evaluated_id"}

    _columns = {
        'vote_evaluated_id': fields.many2one(
            'exchange_rating.evaluated', 'Evaluated',
            ondelete="cascade", required=True
        ),
        'vote_ids': fields.one2many('exchange_rating.exchange_rating', 'partner_id', 'Votes')
    }
