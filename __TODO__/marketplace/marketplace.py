...
class MarketplaceAnnouncementCategory(orm.Model):
    ...
    _inherit = ['exchange_rating.category', 'base.recursive.model']
    _columns = {
        'name': fields.char(
...
class MarketplaceAnnouncement(orm.Model):
    ...
    _inherit = ['mail.thread', 'exchange_rating.model']
    ...
    _columns = {
        ...
        'vote_evaluated_id': fields.many2one(
            'exchange_rating.evaluated', 'Evaluated', ondelete="cascade", required=True
        ),
...
class MarketplaceProposition(orm.Model):
    ...
    def _get_vote_voters(self, cr, uid, ids, name, args, context=None):
        # Get the list of partner which must exchange_rating in
    ...
    _columns = {
        ...
        'skip_vote': fields.boolean('Skip exchange_rating?'),
        'state': fields.selection(
            [
                ('draft', 'Draft'),
                ('open', 'Open'),
                ...
                ('exchange_rating', 'Waiting for votes'),
                ('paid', 'Paid'),
                (
                    'confirm_refund',
    ...
    def test_vote(self, cr, uid, ids, context=None):
        ...
        vote_obj = self.pool.get('exchange_rating.exchange_rating')
    ...
    def confirm(self, cr, uid, ids, *args):
        ...
        # and go to the waiting exchange_rating state
        transaction_obj = self.pool.get('account.wallet.transaction')
    ...
    def write(self, cr, uid, ids, vals, context=None):
        # Security control the want_cancel checkbox and trigger the exchange_rating test
        ...
        for proposition in self.browse(cr, uid, ids, context=context):
            if proposition.state == 'exchange_rating':
